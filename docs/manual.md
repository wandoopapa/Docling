...
```
3. 설치 GUI의 **GPU 사전 점검 버튼**으로 드라이버/CUDA 인식 상태를 먼저 확인합니다.
...
### 4.1 GUI 실행
```bash
.venv\Scripts\pythonw.exe pdf_indexer_app.pyw
```

### 4.2 콘솔 디버그 실행(권장)
```bash
.venv\Scripts\python.exe pdf_indexer_app.pyw
```

### 4.3 설치 완료 후 실행 파일
- `실행하기(무중단).vbs` (사용자 실행용 단일 파일)
- `pdf_indexer_app.pyw` (실제 PDF 변환 실행기, 내부 실행 대상)
- 자동 재시작은 최대 20회로 제한됩니다(무한 재시작 방지).
- 정상 종료 시에는 자동 재시작이 동작하지 않습니다. (토큰+플래그 기반)
- 자동 재시작 OFF 시에는 비정상 종료여도 재시작하지 않습니다.
...
3. 옵션 설정(강제 OCR/병렬 처리/자동 재시작)
   - 현재 안정성 패치로 병렬 옵션 선택 시에도 안정 모드(단일 스레드)로 처리됩니다.
   - 좌측 폴더 트리에서 인덱싱/업데이트 리스트/격리 실패/설정 폴더를 더블클릭으로 열 수 있습니다.
4. 변환 시작
...
- 실행 상태 파일: 실행 폴더의 `run_state.json` (정상/비정상 종료 판정)
- 변환 중 치명적 오류 로그: 출력 폴더의 `crash_traceback.log`
- GPU Torch 설치 재시도(최대 3회): cu121 → cu124 → extra-index fallback
- 설치 후 CUDA 로딩 검증(`torch.cuda.is_available`)이 실패하면 설치를 중단합니다.
- 설치 중 GPU Torch 설치가 실패하면 GPU 설치를 자동 재시도하며, 끝까지 실패하면 설치를 중단합니다.
...
## 11) 최근 안정성 패치
- v42.6: 변환 스레드 예외 안전복구 + UI 메인스레드 위임 + `crash_traceback.log` 저장

- v42.7: 안정 모드(단일 스레드 강제) + 파일 실패 시 엔진 재초기화 1회 재시도

- v42.8: run_state 기반 복구 판정 정교화 + 좌측 폴더 트리 추가

- v42.9: stale 상태 차단(72시간), 파일명 해시 충돌 방지, 0바이트 PDF 격리, 저장 실패 검출, GPU 웜업/주기적 메모리 정리

- v43.0: 좌측 메뉴 페이지 전환(인덱싱/설정/업데이트 리스트/격리·실패), 변환 시작 버튼 가시성 복구
