...
### 4.4 설치 파일 다운로드(실행 가능 버전)
- `install_manager.pyw` 파일(실행 가능 버전)을 저장소 루트에서 다운로드
- 실행: `python install_manager.pyw` 또는 더블클릭
...
- v42.9: stale 상태 차단(72시간), 파일명 충돌 방지, 0바이트 PDF 격리, 저장 실패 감지, GPU 웜업/주기적 캐시 정리
- v43.0: 좌측 메뉴 페이지 전환(인덱싱/설정/업데이트 리스트/격리·실패), 변환 시작 버튼 가시성 복구
- v42.7: 안정 모드(단일 스레드 강제) + 파일 실패 시 엔진 재초기화 1회 재시도
- v42.6: 변환 중 안정성 강화(UI 메인스레드 위임, 치명 예외 로그 저장)
- v42.5: 실행 파일 단일화 + 정상 종료 판정 강화 + GPU 점검 오류 수정
...
아래 코드는 이전 버전에서 제거되었던 설치 코드 블록을 **원문 그대로** 복원한 내용입니다.
...
## 11) 설치 프로그램 무반응(클릭해도 반응 없음) 대응 절차
...
3. 파일 연결 재지정
- `install_manager.pyw` 우클릭 → 연결 프로그램 → `pythonw.exe`/`python.exe`

4. 보안 정책 확인
- 파일 속성의 차단 해제
- 백신/Defender 격리 여부 점검

5. 설치기 import 점검
- 설치기에는 표준 라이브러리만 import
- `psutil`, `torch`, `docling` import는 실행기로 이동

6. 부트 로그 수집
```bash
python install_manager.pyw > installer_boot.log 2>&1
```
- 첫 Traceback 라인을 기준으로 원인 조치


> 안내: 현재 설치기가 생성하는 `runner_app.pyw`는 상태 확인용 더미 창이 아니라 PDF 변환(.md/.txt 출력) 실행기입니다.
