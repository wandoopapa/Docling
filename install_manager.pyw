...
HISTORY_TEXT = """
[v43.0] 페이지 네비게이션 재구성
 - 좌측 메뉴를 '인덱싱/설정/업데이트 리스트/격리·실패 목록' 페이지 리스트로 재구성.
 - 페이지 전환 방식으로 '변환 시작' 버튼 가시성을 복구하고, 인덱싱 화면에서 항상 시작/중단 가능하도록 정리.
 - 각 페이지에서 관련 폴더를 직접 열 수 있도록 버튼 제공(인덱싱/설정/업데이트/격리).

[v42.9] 안정성 전면 개선
 - run_state.json에 PID/타임스탬프를 기록하고 복구 가능 시간(72시간) 검사로 첫 실행 오탐 복구를 차단.
 - PDF 목록 정렬/0바이트 파일 격리/빈 입력 폴더 경고로 작업 흐름 예측 가능성 강화.
 - 원본 파일명 충돌 방지를 위해 해시 접미사 추가(동일 파일명 덮어쓰기 방지).
 - 파일 저장 원자쓰기 실패를 명시적으로 오류 처리하여 손상 출력 누락 방지.
 - 변환 루프 주기적 GC + CUDA 캐시 정리(10개 단위)로 장시간 처리 안정성 강화.
 - GPU 웜업 텐서 연산 추가로 초기 CUDA 상태 이상 조기 감지.
 - fatal_error.flag 기록으로 런처 측 비정상 종료 판단 정확도 향상.
 - 설치 시 이전 복구 토큰/상태 파일 정리로 업데이트 직후 오탐 자동시작 방지.

[v42.8] 복구 판정 정교화 + 폴더 트리
 - run_state.json 기반으로 첫 실행/정상종료/비정상종료를 확정 분리.
 - 복구 자동 시작 조건을 토큰 + 상태값으로 강화하여 오탐 재시작 방지.
 - GUI 좌측에 인덱싱/업데이트 리스트/격리 실패 목록/설정 폴더 트리 및 바로 열기 기능 추가.

[v42.7] 변환 엔진 안정 모드
 - 병렬 처리 옵션 선택 시에도 안정 모드에서 단일 스레드로 강제 전환(크래시 감소 목적).
 - 파일 단위 변환 실패 시 엔진을 1회 재초기화 후 재시도하여 연쇄 종료 가능성 완화.

[v42.6] 변환 중 안정성 강화
 - 백그라운드 처리 스레드 예외를 최상위에서 포착하고 로그/알림 후 안전 복구.
 - Tkinter UI 호출을 메인 스레드로 위임하여 변환 중 비정상 종료 위험 완화.
 - 치명적 예외 발생 시 crash_traceback.log 저장으로 원인 추적 강화.

[v42.5] 단일 실행 파일
 - 설치 결과 실행 파일을 '실행하기(무중단).vbs' 1개로 통합.
 - 기존 배치/별칭 파일은 설치 시 정리.

[v42.4] 정상 종료 판정 강화
 - normal_exit.flag 토큰 도입으로 정상 종료/중단 시 재시작 차단.
 - auto_restart.flag + recovery.token + normal_exit.flag 조합으로 재시작 제어.

[v42.3] GPU 사전 점검 안정화
 - 설치 GUI 점검 버튼 오류(NameError: shutil) 수정.
 - 점검 결과를 사용자에게 명확히 표시.
...
class PDFIndexerApp:
    def __init__(self, root):
        ...
        self.config_file = os.path.join(self.runtime_dir, "indexer_config.json")
        self.token_file = os.path.join(self.runtime_dir, "recovery.token")
        self.normal_exit_file = os.path.join(self.runtime_dir, "normal_exit.flag")
        self.auto_restart_flag_file = os.path.join(self.runtime_dir, "auto_restart.flag")
        self.run_state_file = os.path.join(self.runtime_dir, "run_state.json")
        self.fatal_error_file = os.path.join(self.runtime_dir, "fatal_error.flag")

        self.setup_ui()
        self.cleanup_normal_exit_flag()
        self.load_config()
    ...
    def set_run_state(self, state):
        try:
            payload = {
                ...
                "pid": os.getpid(),
                "auto_restart": bool(self.var_auto_restart.get()),
            }
    ...
    def get_run_state(self):
        try:
            if os.path.exists(self.run_state_file):
            ...
            pass
        return {"state": "idle", "ts": None, "pid": None, "auto_restart": bool(self.var_auto_restart.get())}

    def is_recent_state(self, ts_iso, max_hours=72):
        try:
            if not ts_iso:
                return False
            saved = datetime.fromisoformat(ts_iso)
            return (datetime.now() - saved).total_seconds() <= max_hours * 3600
        except Exception:
            return False
    ...
    def should_auto_resume(self):
        if not self.var_auto_restart.get():
            return False
        if not (self.input_dir.get() and self.output_dir.get()):
            return False
        if not os.path.exists(self.token_file):
            return False
        if os.path.exists(self.normal_exit_file):
            return False
        state_data = self.get_run_state()
        state = state_data.get("state", "idle")
        ts_iso = state_data.get("ts")
        return state in {"running", "crash"} and self.is_recent_state(ts_iso, max_hours=72)
    ...
    def mark_normal_exit(self):
        try:
            with open(self.normal_exit_file, "w", encoding="utf-8") as f:
                f.write("normal")
        except Exception:
            pass

    def cleanup_normal_exit_flag(self):
        try:
            if os.path.exists(self.normal_exit_file):
                os.remove(self.normal_exit_file)
        except Exception:
            pass

    def mark_fatal_error(self, message="fatal"):
        try:
            with open(self.fatal_error_file, "w", encoding="utf-8") as f:
                f.write(f"{datetime.now().isoformat()} | {message}\n")
        except Exception:
            pass

    def clear_fatal_error_flag(self):
        try:
            if os.path.exists(self.fatal_error_file):
                os.remove(self.fatal_error_file)
        except Exception:
            pass
    ...
    def on_closing(self):
        if self.is_running:
            if not messagebox.askyesno("종료", "작업이 진행 중입니다. 정말 종료하시겠습니까?"):
                return
        self.mark_normal_exit()
        self.cleanup_token()
        self.clear_fatal_error_flag()
        self.set_run_state("normal_exit")
        self.root.destroy()
        sys.exit(0)
    ...
    def safe_messagebox(self, title, text, kind="info"):
        def _show():
            if kind == "error":
    ...
    def update_restart_flag(self):
        try:
            if self.var_auto_restart.get():
                with open(self.auto_restart_flag_file, "w", encoding="utf-8") as f:
                    f.write("ON")
            elif os.path.exists(self.auto_restart_flag_file):
                os.remove(self.auto_restart_flag_file)
        except Exception as e:
    ...
    def setup_ui(self):
        root_wrap = tk.Frame(self.root, bg="#ecf0f1")
        root_wrap.pack(fill="both", expand=True)

        nav = tk.Frame(root_wrap, bg="#2c3e50", width=220)
        nav.pack(side="left", fill="y")
        nav.pack_propagate(False)

        content = tk.Frame(root_wrap, bg="#fdfefe")
        content.pack(side="left", fill="both", expand=True)

        tk.Label(nav, text="메뉴", bg="#2c3e50", fg="white", font=("Malgun Gothic", 14, "bold")).pack(anchor="w", padx=14, pady=(16, 10))
        ...
        for name in menu_items:
            btn = tk.Button(nav, text=name, anchor="w", relief="flat", bd=0, bg="#34495e", fg="white",
                            activebackground="#1abc9c", activeforeground="white",
                            font=("Malgun Gothic", 11, "bold"), padx=12, pady=10,
                            command=lambda n=name: self.show_page(n))
            btn.pack(fill="x", padx=10, pady=4)
            self.nav_buttons[name] = btn

        top_header = tk.Frame(content, bg="#fdfefe", padx=18, pady=14)
        top_header.pack(fill="x")
        tk.Label(top_header, text="PDF 변환 컨트롤러", font=("Malgun Gothic", 20, "bold"), bg="#fdfefe", fg="#2c3e50").pack(side="left")
        self.lbl_gpu = tk.Label(top_header, text="GPU: 확인 중...", font=("Malgun Gothic", 10, "bold"), bg="#ecf0f1", fg="#7f8c8d", padx=10, pady=5, relief="ridge")
        self.lbl_gpu.pack(side="right")

        self.page_host = tk.Frame(content, bg="#fdfefe", padx=16, pady=12)
        self.page_host.pack(fill="both", expand=True)
        self.page_frames = {}

        # 인덱싱 페이지
        page_index = tk.Frame(self.page_host, bg="#fdfefe")
        self.page_frames["인덱싱 페이지"] = page_index

        tk.Label(page_index, text="1. 원본 PDF 폴더:", font=("Malgun Gothic", 10, "bold"), bg="#fdfefe").pack(anchor="w")
        f1 = tk.Frame(page_index, bg="#fdfefe")
        f1.pack(fill="x", pady=5)
        tk.Entry(f1, textvariable=self.input_dir, state="readonly", bg="white").pack(side="left", fill="x", expand=True, ipady=3)
        tk.Button(f1, text="폴더 찾기", command=self.select_input_dir, bg="#ecf0f1").pack(side="left", padx=(8, 0))
        tk.Button(f1, text="폴더 열기", command=lambda: self.open_folder(self.input_dir.get()), bg="#dfe6e9").pack(side="left", padx=(6, 0))

        tk.Label(page_index, text="2. 데이터 저장 폴더:", font=("Malgun Gothic", 10, "bold"), bg="#fdfefe").pack(anchor="w", pady=(8, 0))
        f2 = tk.Frame(page_index, bg="#fdfefe")
        f2.pack(fill="x", pady=5)
        tk.Entry(f2, textvariable=self.output_dir, state="readonly", bg="white").pack(side="left", fill="x", expand=True, ipady=3)
        tk.Button(f2, text="폴더 찾기", command=self.select_output_dir, bg="#ecf0f1").pack(side="left", padx=(8, 0))
        tk.Button(f2, text="폴더 열기", command=lambda: self.open_folder(self.output_dir.get()), bg="#dfe6e9").pack(side="left", padx=(6, 0))

        st = tk.LabelFrame(page_index, text="작업 대시보드", bg="#fdfefe", padx=10, pady=5)
        ...
        tk.Label(page_index, text="실시간 로그:", font=("Malgun Gothic", 9, "bold"), bg="#fdfefe").pack(anchor="w")
        self.txt_log = tk.Text(page_index, height=13, state="disabled", font=("Consolas", 9), bg="#f4f6f7")
        self.txt_log.pack(fill="both", expand=True, pady=5)
        self.btn_start = tk.Button(page_index, text="변환 시작 (Start)", command=self.toggle_processing, bg="#27ae60", fg="white", font=("Malgun Gothic", 14, "bold"), height=2)
        self.btn_start.pack(fill="x", pady=10)

        # 설정 페이지
        page_settings = tk.Frame(self.page_host, bg="#fdfefe")
        self.page_frames["설정 페이지"] = page_settings
        opt = tk.LabelFrame(page_settings, text="설정 (Settings)", bg="#fdfefe", padx=12, pady=10)
        opt.pack(fill="x", pady=(0, 10))
        tk.Checkbutton(opt, text="일본어/한자 유지", variable=self.var_keep_hanja, bg="#fdfefe", font=("Malgun Gothic", 10)).pack(anchor="w")
        tk.Checkbutton(opt, text="고밀도 정밀 분석 (강제 OCR) - 품질↑ 속도↓", variable=self.var_force_ocr, bg="#fdfefe", fg="#c0392b", font=("Malgun Gothic", 10, "bold")).pack(anchor="w")
        tk.Checkbutton(opt, text="고속 병렬 처리 (2-Threads) - 속도↑ 메모리↑", variable=self.var_parallel, bg="#fdfefe", fg="#2980b9", font=("Malgun Gothic", 10, "bold")).pack(anchor="w")
        tk.Checkbutton(opt, text="오류 발생 시 자동 재시작 (Auto-Recovery)", variable=self.var_auto_restart, command=self.on_auto_restart_toggle, bg="#fdfefe", fg="#e67e22", font=("Malgun Gothic", 10, "bold")).pack(anchor="w")
        action = tk.Frame(page_settings, bg="#fdfefe")
        action.pack(fill="x", pady=(4, 8))
        tk.Button(action, text="설정 저장", bg="#16a085", fg="white", font=("Malgun Gothic", 10, "bold"), command=self.save_config).pack(side="left")
        tk.Button(action, text="설정 폴더 열기", bg="#dfe6e9", command=lambda: self.open_folder(self.runtime_dir)).pack(side="left", padx=(8, 0))
        tk.Label(page_settings, text="설정 파일: indexer_config.json / auto_restart.flag / run_state.json", bg="#fdfefe", fg="#7f8c8d", font=("Malgun Gothic", 9)).pack(anchor="w")

        # 업데이트 리스트 페이지
        page_updates = tk.Frame(self.page_host, bg="#ecf0f1")
        self.page_frames["업데이트 리스트 페이지"] = page_updates
        top_u = tk.Frame(page_updates, bg="#ecf0f1")
        top_u.pack(fill="x", pady=(0, 8))
        tk.Label(top_u, text="📘 업데이트 리스트 (Updates)", font=("Malgun Gothic", 14, "bold"), bg="#ecf0f1", fg="#27ae60").pack(side="left")
        tk.Button(top_u, text="업데이트 폴더 열기", command=lambda: self.open_folder(self.runtime_dir), bg="#dfe6e9").pack(side="right")
        self.txt_history = scrolledtext.ScrolledText(page_updates, font=("Malgun Gothic", 9), bg="white", relief="flat")
        ...
        # 격리/실패 목록 페이지
        page_errors = tk.Frame(self.page_host, bg="#ecf0f1")
        self.page_frames["격리/실패 목록 페이지"] = page_errors
        top_e = tk.Frame(page_errors, bg="#ecf0f1")
        top_e.pack(fill="x", pady=(0, 8))
        tk.Label(top_e, text="❌ 격리/실패 목록", font=("Malgun Gothic", 13, "bold"), bg="#ecf0f1", fg="#c62828").pack(side="left")
        tk.Button(top_e, text="격리 폴더 열기", command=lambda: self.open_folder(os.path.join(self.output_dir.get(), "_QUARANTINE_LOW_QUALITY") if self.output_dir.get() else ""), bg="#dfe6e9").pack(side="right")
        self.lst_errors = tk.Listbox(page_errors, font=("Consolas", 10), bg="white", fg="#c62828")
        self.lst_errors.pack(fill="both", expand=True)

        self.show_page("인덱싱 페이지")

    def show_page(self, page_name):
        for name, frame in self.page_frames.items():
            if name == page_name:
                frame.pack(fill="both", expand=True)
                self.nav_buttons[name].configure(bg="#1abc9c")
            else:
                frame.pack_forget()
                self.nav_buttons[name].configure(bg="#34495e")
    ...
    def process_manager(self):
        ...
        try:
            ...
            if self.converter is None:
                ...
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        # GPU warm-up (early CUDA failure detection)
                        _warm = torch.zeros((1, 1), device="cuda")
                        del _warm
            ...
            if self.total_files == 0:
                ...
                self.cleanup_token()
                self.set_run_state("normal_exit")
                self.safe_messagebox("안내", "선택한 폴더에 PDF 파일이 없습니다.", kind="warning")
                return
            ...
            if self.var_parallel.get():
                self.log_gui("⚠️ 안정 모드 활성화: 병렬 옵션이 선택되어도 단일 스레드로 처리합니다.")

            self.log_gui("🐢 안정 처리(1-Thread) 모드로 시작합니다.")
            ...
            if not self.stop_event.is_set():
                ...
                self.run_on_ui(lambda: self.var_auto_restart.set(False))
                self.save_config()
                self.update_restart_flag()
                self.mark_normal_exit()
                self.cleanup_token()
                self.set_run_state("normal_exit")
                self.safe_messagebox("완료", "모든 작업이 끝났습니다.")
            else:
                ...
                self.cleanup_token()
                self.set_run_state("normal_exit")
                self.safe_messagebox("중단", "작업이 중단되었습니다.")

        except Exception as e:
            self.log_gui(f"❌ 치명적 예외 발생: {e}")
            self.mark_fatal_error(str(e))
            try:
                with open(os.path.join(self.output_dir.get() or os.getcwd(), "crash_traceback.log"), "a", encoding="utf-8") as cf:
            ...
            if self.var_auto_restart.get():
                ...
                self.cleanup_token()
                self.set_run_state("normal_exit")
            self.safe_messagebox("오류", f"변환 중 치명적 오류가 발생했습니다.\n{e}\n\ncrash_traceback.log를 확인하세요.", kind="error")
        finally:
            self.allow_sleep()
            self.run_on_ui(self.reset_ui)
    ...
    def process_single_file(self, full_path, q_dir):
        ...
        unique_name = f"{clean_name}_{phash}"
        md_path = os.path.join(self.output_dir.get(), f"{unique_name}.md")
        q_path = os.path.join(q_dir, f"{unique_name}.md")
        ...
        if os.path.getsize(full_path) == 0:
            self.log_gui(f"[격리] {fname} (0바이트 PDF)")
            self.add_error_log(fname, "Zero-byte PDF")
            self.append_manifest(fname, phash, "QUARANTINED", 0, "ZeroByte")
        ...
        try:
            ...
            if not self.write_atomic(target_file, final_text):
                raise RuntimeError(f"파일 저장 실패: {target_file}")
            try:
                import torch
                if torch.cuda.is_available():
                    torch.cuda.empty_cache()
            ...
            self.log_gui(f"[재시도 준비] {fname} 1차 실패: {e}")
            retry_ok = False
            try:
                self.log_gui("♻️ 변환 엔진 재초기화 후 1회 재시도...")
                self.converter = None
                gc.collect()
                try:
                    import torch
                ...
                if (fsize > 5 and tlen < 200) or (tlen < 50):
                    ...
                    self.log_gui(f"[완료] {fname} (재시도 성공)")
                    self.append_manifest(fname, phash, "SUCCESS", tlen, "RetriedOnce")

                self.write_atomic(target_file, final_text)
                retry_ok = True
            except Exception as e2:
                self.log_gui(f"[실패] {fname} 재시도 실패: {e2}")
...
class InstallerGUI:
    ...
    def run_install(self):
        try:
            ...
            for stale_name in ["recovery.token", "normal_exit.flag", "auto_restart.flag", "run_state.json", "fatal_error.flag"]:
                stale_path = os.path.join(target, stale_name)
                try:
...
if exist "normal_exit.flag" (
    echo [System] 정상 종료 토큰 감지. 자동 재시작하지 않습니다.
    del /f /q "normal_exit.flag" >nul 2>nul
    exit /b 0
)
if %errorlevel% neq 0 (
    if exist "fatal_error.flag" (
        echo [System] 치명적 오류 플래그 감지. 자동 복구 절차를 진행합니다.
        del /f /q "fatal_error.flag" >nul 2>nul
    )
...
'''
            ...
            # 실행 파일 1개 정책: legacy 런처 정리
            for legacy in ["RUN_DOCLING.bat", "RUN_DOCILING.bat"]:
                legacy_path = os.path.join(target, legacy)
                try:
