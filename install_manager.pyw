...
- Standard-library-only launcher
- Creates venv and installs runtime deps
"""
...
class InstallerApp:
    ...
    def run_install(self) -> None:
        try:
            ...
            self._set_status("5/5 실행 스크립트 생성 중...", 40)
            launch = target / "RUN_DOCILING.bat"
            launch.write_text(
                f'@echo off\n"{venv_python}" -m pip --version\n'
                f'echo 설치 완료. 실행 앱 경로를 연결하세요.\n',
                encoding="utf-8",
            )
            self.current_step = 5
            self._set_status("5/5 설치 완료", 100)

            messagebox.showinfo("설치 완료", f"설치가 완료되었습니다.\n경로: {target}")
