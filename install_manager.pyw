...
class PDFIndexerApp:
    ...
    def setup_ui(self):
        ...
        tk.Button(f1, text="폴더 찾기", command=self.select_input_dir, bg="#ecf0f1").pack(side="right")
        ...
        tk.Button(f2, text="폴더 찾기", command=self.select_output_dir, bg="#ecf0f1").pack(side="right")

        tree_box = tk.LabelFrame(left, text="폴더 트리", bg="#fdfefe", padx=8, pady=6)
        tree_box.pack(fill="x", pady=(8, 10))
        self.folder_tree = ttk.Treeview(tree_box, height=6)
        self.folder_tree.pack(fill="x")
        self.folder_tree.bind("<Double-1>", self.open_selected_folder)
        tk.Button(tree_box, text="트리 새로고침", command=self.refresh_folder_tree, bg="#ecf0f1").pack(anchor="e", pady=(4, 0))
        ...
        tk.Label(right, text="📘 업데이트 리스트 (Updates)", font=("Malgun Gothic", 14, "bold"), bg="#ecf0f1", fg="#27ae60").pack(anchor="w", pady=(0, 10))
    ...
    def refresh_folder_tree(self):
        try:
            ...
            folders = {
                "인덱싱 폴더": self.input_dir.get() or "(미설정)",
                "업데이트 리스트 폴더": self.runtime_dir,
                "격리 실패 목록 폴더": os.path.join(self.output_dir.get(), "_QUARANTINE_LOW_QUALITY") if self.output_dir.get() else "(미설정)",
                "설정 폴더": self.runtime_dir,
            }
            for label, path in folders.items():
                node = self.folder_tree.insert("", "end", text=label, values=(path,))
                self.folder_tree.insert(node, "end", text=path)
        except Exception as e:
            self.log_gui(f"⚠️ 폴더 트리 갱신 실패: {e}")
    ...
    def open_selected_folder(self, _event=None):
        try:
            ...
            if not text:
                return
            path = text
            if text in ["인덱싱 폴더", "업데이트 리스트 폴더", "격리 실패 목록 폴더", "설정 폴더"]:
                children = self.folder_tree.get_children(item)
                if children:
                    path = self.folder_tree.item(children[0], "text")
